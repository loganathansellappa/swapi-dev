...
export const PersonInfo = () => {
  ...
  const getProperties = (person: ResourceType) => {
    ...
    const peopleType = ['people', 'residents', 'characters'];
    for (const key in person) {
      if (
        [...urls, ...peopleType].includes(key) &&
        person[key as keyof ResourceType]
      ) {
        const links = [];
        if (Array.isArray(person[key as keyof ResourceType])) {
          const resourceKey = peopleType.includes(key) ? 'people' : key;
          person[key as keyof ResourceType].forEach<AnyResourceType>(
            (resourceUrl: string, index: number) =>
              links.push(
                <Link
                  className={'resourceLink'}
                  to={`/resource/${resourceKey}/${resourceUrl
                    .match(/\d/g)
                    .join('')}`}
                >
                  {capitalize(key)}
          ...
          links.push(
            <Link
              className={'todo'}
              to={`/resource/${key}/${person[key as keyof ResourceType]
                .match(/\d/g)
                .join('')}`}
            />
          );
  ...
  return (
    <div className="container">
      ...
      <div className="personDetail">
        {getProperties(person as unknown as ResourceType)}
      </div>
    </div>
  );
};
