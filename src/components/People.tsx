...
const People: React.FunctionComponent = () => {
  ...
  const peopleData = useGetResourceQuery(
    `people?search=${search}&page=${page}`
  );
  const planetsData = useGetResourceQuery(
    `planets?search=${search}&page=${page}`,
    { skip: !search.length }
  );
  const shipData = useGetResourceQuery(
  ...
  useMemo(() => {
    ...
    const validData = search.length
      ? [peopleData, planetsData, shipData]
      : [peopleData];
    for (const {
      currentData: data,
      error: err,
    ...
    <div className="PeopleContainer">
      <div className="search">
        <input
          ...
          value={search}
          placeholder="Search People/Planet/StarShips.."
          onChange={(e) => updateSearch((e.target as HTMLInputElement).value)}
        />
      </div>
      {loading ? (
        <Loader size={100} className={'inline'} />
      ) : (
