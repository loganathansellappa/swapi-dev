...
interface PersonProps {
    person: AnyResourceType
}

export const Person = ({person} : PersonProps) => {
    const dispatch = useDispatch()
    const isFavourite = useSelector((state: RootState) => state.favoritePeople.favourites?.find((p) => p.url === person.url))
    // @ts-ignore
    ...
    const typeInfo = (data : AnyResourceType) => {
        if(data.episode_id) { return 'Film'}
        if(data.rotation_period) { return 'Planet'}
        if(data.manufacturer) { return 'StarShip'}
        if(data.vehicle_class) { return 'Vehicle'}
        return 'People'
    }
    ...
    return(
                <div className='personInfo'>
                    ...
                    <div className='favourite' onClick={setFavourite}>
                        ...
                        <div><BsFillHeartFill size={20} color={isFavourite ? 'red' : 'white'} /></div>
                    </div>
                </div>
    )
}